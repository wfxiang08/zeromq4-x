...
void zmq::stream_engine_t::out_event() {
    ...
    if (!outsize) {
        ...
        while (outsize < out_batch_size) {
            ...
            // 如果encoder中没有有效的数据，那么outpos == NULL
            if (outpos == NULL)
                outpos = bufptr;
            outsize += n;
        }
    ...
    if (unlikely (handshaking)) if (outsize == 0)
        reset_pollout(handle);
}
...
//
// 存在两个概念: session vs. socket, 其中session是服务器本地的状态, 存在缓存
//
