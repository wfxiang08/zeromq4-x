...
// 注意: socket_base_t 和 ctx_t中的create函数的区别, 后者做了很多准备工作
//
zmq::socket_base_t *zmq::ctx_t::create_socket(int type_) {
    slot_sync.lock();
    
    // 虽然是unlikely, 但是也是必须执行的一次
    if (unlikely (starting)) {
        ...
        // 1. 创建 slots
        //    这么多slots做什么用? 为什么每个socket对应一个mailbox
        slot_count = mazmq + ios + 2;
        slots = (mailbox_t **) malloc(sizeof(mailbox_t * ) * slot_count);
        alloc_assert (slots);

        // 2. 两类特殊的 thread/特殊的mailbox, 也就是: slot_count中出现的 2
        //    term thread & reaper thread
        ...
        // XXX: 注意关注: reaper->start, io_thread->start()
        // 它启动了 poller的 event loop, 对fd进行了监控，并且处理了time event

        //  Create I/O thread objects and launch them.
        // 创建更多的io thread, 默认只有一个, 基本就够用了
        for (int i = 2; i != ios + 2; i++) {
            io_thread_t *io_thread = new(std::nothrow) io_thread_t(this, i);
            alloc_assert (io_thread);
            io_threads.push_back(io_thread);
            slots[i] = io_thread->get_mailbox();
            io_thread->start(); // 开始监控fd的变化
        }

        //  In the unused part of the slot array, create a list of empty slots.
        // 将准备给socket的 slots清空，并且添加到 empty_slots中
        //
    ...
    // 1. 首先处理各种异常清空: context关闭，slots不够用
    // context关闭之后就不能再用了
    ...
    // 3. socket创建成功, 保存sockets, 安排slots
    sockets.push_back(s);
    
    // socket中的静态变量: 
    // mailbox --> yqueue_t 固定存在的
    // 
    // slots中保存mailbox
...
void zmq::ctx_t::destroy_socket(class socket_base_t *socket_) {
    ...
    // 1. Free the associated thread slot.
    ...
    //  2. If zmq_ctx_term() was already called and there are no more socket
...
//
// 发送command到指定的 slots, 每个slot对应一个thread?
//
...
void zmq::ctx_t::connect_pending(const char *addr_, zmq::socket_base_t *bind_socket_) {
    ...
    std::pair<pending_connections_t::iterator, pending_connections_t::iterator> pending = pending_connections.equal_range(
            addr_);
...
void zmq::ctx_t::connect_inproc_sockets(zmq::socket_base_t *bind_socket_, options_t &bind_options,
                                        pending_connection_t &pending_connection_, side side_) {
